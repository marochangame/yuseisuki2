(() => {
  ...
  function playStartThenShowGame() {
    ...
    speak("はじめるよー！", { rate: 0.95, pitch: 1.12, volume: 1, cancel: true, onend: switchToGame });
    setTimeout(switchToGame, 1400);
    beep(784, 0.06, 0.04, 0.02);
    beep(1046, 0.08, 0.24, 0.018);
  }
  ...
  function playChoiceSound(chosenName) {
    beep(880, 0.07, 0.00, 0.08);
    beep(1175, 0.09, 0.09, 0.075);
    beep(1568, 0.14, 0.21, 0.055);
    const line = "いいねぇ！";
    setTimeout(() => {
      speak(`${chosenName}、いいねぇ！`, { rate: 1.03, pitch: 1.18, volume: 1, cancel: true });
    }, 190);
    return line;
  }
  ...
  function renderPair() {
    ...
    adultPair.textContent = `${questionCount}/${MAX_QUESTIONS}：${currentPair[0].name} / ${currentPair[1].name}`;
  }
  ...
  function showFinish() {
    ...
    setTimeout(() => {
      speak("おしまい！またあそぼうね！", { rate: 0.96, pitch: 1.18, volume: 1, cancel: true });
    }, 260);
    setTimeout(() => {
